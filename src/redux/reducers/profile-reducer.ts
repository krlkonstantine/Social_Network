...
import {
  ProfilePropsType,
  ProfileType,
} from "../../components/main/ProfilePage/Profile";
...
export const addPostActionCreator = (textNewPost: string) =>
  ({ type: ADD_POST, textNewPost: textNewPost }) as const;
...
export const getProfile =
  (userId: string | null) => async (dispatch: Dispatch<ActionType>) => {
...
export const getStatus =
  (userId: string) => async (dispatch: Dispatch<ActionType>) => {
    dispatch(changePreloaderStatus(true));
...
export const updateStatus =
  (status: string) => async (dispatch: Dispatch<ActionType>) => {
    dispatch(changePreloaderStatus(true));
    try {
...
export const uploadNewProfilePhoto =
  (photo: File) => async (dispatch: Dispatch<ActionType>) => {
    dispatch(changePreloaderStatus(true));
    const res = await profileAPI.uploadPhoto(photo);
    if (res.resultCode === 0) dispatch(savePhotoSuccess(res.data.photos));
    dispatch(changePreloaderStatus(false));
  };
export const saveNewProfileInfo =
  (formData: ApiUserProfileType) =>
  async (
    dispatch: ThunkDispatch<ApiUserProfileType, unknown, AnyAction>,
    ...
    if (res.resultCode === 0) {
      ...
      dispatch(
        stopSubmit("profile", {
          _error:
            res.messages.length > 0 ? res.messages[0] : "Some error occured",
        }),
      );
